...
/// Thank you Andrew Kelly
/// https://github.com/andrewrk/StaticHttpFileServer
/// THIS IS DEPRECATED
/// Only keeping in library for reference, will be fully removed eventually
const Self = @This();

/// The key is index into backing_memory, where a HTTP request path is stored.
files: File.Table,
/// This matches the pattern in `cache.zig`
/// Duplication seemed fine
files_mrc: std.atomic.Value(u64),
files_should_update: std.atomic.Value(bool),
...
pub fn init(options: Options) InitError!Self {
    ...
    return .{
        .files = files,
        .bytes = bytes,
        .files_mrc = std.atomic.Value(u64).init(root.computeFolderMRC(options.dir_path) catch @panic("failed to get mrc")),
        .files_should_update = std.atomic.Value(bool).init(false),
    };
}
...
pub fn initWatchChangesThread(self: *Self) void {
    const thread = std.Thread.spawn(.{}, filesUpdateBackgroundWatcher, .{
        &self.files_mrc,
        &self.files_should_update,
    }) catch @panic("failed to spawn watcher thread");
    thread.detach();
}

fn filesUpdateBackgroundWatcher(dir_path: []const u8, mrc_ptr: *std.atomic.Value(u64), update_ptr: *std.atomic.Value(bool)) void {
    while (true) {
        std.Thread.sleep(5_000_000_000); // sleep 5 seconds (nano)
        const new_mrc = root.computeFolderMRC(dir_path) catch continue;
...
pub fn tryUpdate(self: *Self) void {
    if (self.files_should_update.swap(false, .seq_cst)) {
        // update
    }
}
